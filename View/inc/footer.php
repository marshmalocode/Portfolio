<footer class="globalfooter">
    <div class="copyright">
        <p>2020 © <a href="#" target="_blank">c'est copy right </a></p>
    </div>
    <div class="footer-lien">
        <nav class="footer-navigation">
            <ul class="footer-menu">
                <li class="menu-item"><a href="?z=cv">CV</a></li>
                <li class="menu-item"><a href="?z=trav">Travaux</a></li>
                <li class="menu-item"><a href="?z=contact">Contact</a></li>
            </ul>
        </nav>
    </div>
    <button id="dark-mode-toggle" onclick="toggleDarkMode()">
        <span id="toggle-icon" class="sun-icon"></span>
        <span class="on-off-switch"></span>
    </button>
    <div class="footer-socials">
        <a href="" class="icon-footer-socials"><img src="/asset/icon-github.png" alt=""></a>
        <a href="" class="icon-footer-socials"><img src="/asset/icon-linkedin.png" alt=""></a>
        <a href="" class="icon-footer-socials"><img src="/asset/icon-telegram.png" alt=""></a>
        <a href="" class="icon-footer-socials"><img src="/asset/icon-insta.png" alt=""></a>
        <a href="" class="icon-footer-socials"><img src="/asset/icon-user.png" alt=""></a>
    </div>
</footer>